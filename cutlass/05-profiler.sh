...
#
# Quick test (2 pairs, skip “*static*”):
#   OUT_DIR=/abs/out  SKIP_PATTERNS='*static*'  MAX_PAIRS=2  ./05-profiler.sh
#
# CI-ish run (~100 pairs):
#   OUT_DIR=/abs/out  SKIP_PATTERNS='*static*'  MAX_PAIRS=100  ./05-profiler.sh
#
# Key env vars:
#   OUT_DIR (required)     Path holding cutlass/build
#   SKIP_PATTERNS          Colon-separated globs to exclude (e.g. '*static*:*planar_complex*')
#   MAX_PAIRS              Limit number of (op,kernel) pairs (default: 2)
#   PROFILER               Path to cutlass_profiler (default: ./tools/profiler/cutlass_profiler)
#   WARMUP / ITERS         Profiler warmup/iterations (defaults: 5 / 30)
#   VERIFY                 'true' or 'false' (default: false)
#   PROFILER_EXTRA         Extra profiler args (space-separated)
#
# Outputs:
#   $OUT_DIR/cutlass/build/cutlass_profiler.build.log
#   $OUT_DIR/cutlass/build/profiler.log
#   $OUT_DIR/cutlass/build/profiler-report.xml
#   $OUT_DIR/cutlass/build/cutlass-profiler-kernels.csv
#   export OUT_DIR=/home/manos/native_cutlass
#
# Tip: “profiler.log not found” → ensure OUT_DIR matches where this script writes logs.
# =============================================================================
...
MAX_PAIRS="${MAX_PAIRS:-1000}" #Huge number: reflects to 235 operation and kernel-pattern
...
for t in "${TARGETS[@]}"; do
  ...
  key="${op}|${kpat}"
  [[ -z "${SEEN[$key]:-}" ]] && { SEEN[$key]=1; PAIRS+=("$key"); }
done
...
for pair in "${PAIRS[@]}"; do
  ...
  RUNLOG="$(mktemp)"
  set +e
  run_pair "$op" "$kpat" |& tee -a "$LOGFILE" | tee "$RUNLOG" >/dev/null
  rc=${PIPESTATUS[0]}
  set -e
  run_end=$(date +%s.%N)
...
with open(out_csv, 'w', newline='') as f:
    w = csv.writer(f)
    w.writerow(['Kernel','Bytes','FLOPs','FLOPS/Byte','Runtime_ms','Memory_GiB_s','Math_GFLOP_s'])
    for r in rows:
        w.writerow([
            r['Kernel'],
            r['Bytes'],
