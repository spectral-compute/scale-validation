...
for b in "${benchmarks[@]}"; do
    ...
    *)
    ...
    # Grab average GPU time (ms, then seconds for JUnit)
    ...
    if ((rc == 0)); then
        ...
        if [[ " ${known_failures[*]} " == *" $b "* ]]; then
            ...
            log "[   KNOWN FAILURE   ] $b (ignored; see $(basename "$DIFFLOG"))"
        else
            ((++failed))
            short_ms=$(awk -v ms="${ms:-0}" 'BEGIN{printf "%.2f", ms+0}')
    ...
    log "[   AVG GPU time    ] ${ms:-0} ms"
    popd >/dev/null
done

log "[===================] $total apps ran."
log "[       PASSED      ] $passed apps."
log "[       FAILED      ] $failed apps."
log "[       SKIPPED     ] $skipped apps."
